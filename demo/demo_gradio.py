...
import nest_asyncio
import asyncio
# Force default event loop policy to avoid uvloop/nest_asyncio conflicts
...
# Ensure we import the local dots_ocr package, not a system-installed one
sys.path.insert(0, str(root_dir))
print(f"Added {root_dir} to sys.path to ensure local imports.")
...
dots_parser = DotsOCRParser(
    ...
    model_name=os.environ.get("GEMINI_MODEL", "rednote-hilab/dots.ocr")
)
        ...
        for i, result in enumerate(results):
            ...
            if 'layout_info_path' in result and os.path.exists(result['layout_info_path']):
                with open(result['layout_info_path'], 'r', encoding='utf-8') as f:
                    ...
                    if page_result['cells_data'] is not None:
...
def process_image_inference(session_state, file_input,
                          prompt_mode, server_ip, server_port, min_pixels, max_pixels,
                          model_selection, fitz_preprocess=False, request_delay=2.0
                          ):
    ...
    dots_parser.model_name = model_selection
    dots_parser.ip = server_ip
    dots_parser.port = server_port
    dots_parser.min_pixels = min_pixels
    dots_parser.max_pixels = max_pixels
    dots_parser.request_delay = request_delay
    
    input_file_path = file_input
    
    if not input_file_path:
        return None, "Please upload image/PDF file", "", "", gr.update(value=None), None, "", session_state
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme="ocean", css=GRADIO_CSS, title='dots.ocr') as demo:
            ...
            with gr.Column(scale=1, elem_id="left-panel"):
                ...
                with gr.Group():
                    ...
                    model_selection = gr.Dropdown(
                        ...
                        choices=["gemini-2.0-flash-exp", "gpt-4o"],
                        value="gemini-2.0-flash-exp",
                        info="Choose the reasoning engine"
                    )
                    prompt_mode = gr.Dropdown(
                        label="📝 Prompt Template",
                        choices=[
                            "prompt_layout_all_en", 
                            "prompt_inference_markdown",
                            "prompt_cleaning_markdown",
                ...
                with gr.Row():
                    process_btn = gr.Button("🚀 RUN PARSE", variant="primary", scale=2, elem_id="parse_button", size="lg")
                    clear_btn = gr.Button("🗑️ RESET", variant="secondary", scale=1, size="lg")
                ...
                with gr.Accordion("🛠️ Advanced Configuration", open=False):
                    ...
                    with gr.Row():
                         request_delay_input = gr.Number(
                            label="Request Delay (s)", 
                            value=2.0, 
                            precision=1,
                            info="Delay between API requests to avoid rate limits."
                        )
        ...
        # Also handle test image selection - REMOVED
        ...
        process_btn.click(
            ...
            inputs=[
                session_state, file_input,
                prompt_mode, server_ip, server_port, min_pixels, max_pixels, 
                model_selection, fitz_preprocess, request_delay_input
            ],
            outputs=[
